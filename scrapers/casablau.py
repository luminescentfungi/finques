"""
Scraper for casablau.net – rental listings (JS-rendered).

URL: https://casablau.net/alquiler/pisos-en-alquiler
"""
...
BASE   = "https://casablau.net"
SEARCH = f"{BASE}/alquiler/pisos-en-alquiler"


class CasaBlauScraper(PlaywrightBaseScraper):
    name = "casablau"
    base_url = BASE
    ...
    def search(self, params: SearchParams) -> List[Listing]:
        ...
        for page in range(1, params.max_pages + 1):
            ...
            html = self._page_html(url, wait_selector="[class*='property'], article")
            if not html:
                break

            bs = soup(html)
            cards = bs.find_all("a", href=re.compile(r"/(alquiler|inmueble|piso|propiedad)/"))
            if not cards:
                break
            ...
            for card in cards:
                ...
                results.append(
                    self._safe_listing(
                        ...
                        size_m2=size_m2, bedrooms=bedrooms, city="Barcelona",
                    )
                )
                found_any = True
